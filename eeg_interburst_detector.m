%-------------------------------------------------------------------------------
% eeg_interburst_detector: Detect bursts for one (bipolar) channel of EEG
%
...
%     Fs=64; 
%
%     [burst_anno,svm_out]=eeg_interburst_detector(x,Fs);
% 
%     figure(1); clf; 
%     t=(0:N-1)./Fs;
...
%
%-------------------------------------------------------------------------------
function [burst_anno,svm_out,t_stat]=eeg_interburst_detector(eeg_data,Fs)
...
x=bsxfun(@minus,t_stat,[lin_svm_param_st.x_shift]');
x=bsxfun(@times,x,(1./[lin_svm_param_st.x_scale])');

% b) y ~ b + ∑ᵢ wᵢ xᵢ
y=lin_svm_param_st.bias;
for n=1:N_feats
    y=y+lin_svm_param_st.coeff(n).*x(n,:);
end
...
if(STATIC_THRES)
    burst_anno(burst_anno>=0.298279990868823)=1;
    burst_anno(burst_anno<0.298279990868823)=0;
